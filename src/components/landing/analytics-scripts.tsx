...
import { adsId, gtmId } from "@/lib/analytics";
      ...
      {adsId && (
        <>
          <Script
            id="ads-config"
            strategy="afterInteractive"
          >{`window.dataLayer=window.dataLayer||[];window.gtag=function(){dataLayer.push(arguments);};gtag('js',new Date());gtag('config',${JSON.stringify(adsId)});`}</Script>
          <Script
